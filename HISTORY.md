...
### 训练评估系统上正规流程 → 起草需求+验收文档(系列1/4)
- 用户决定给"训练模型"上正规开发流程(需求→产品→开发→验收,充分论证再开工)。
- V1 主张:精而硬非厚而空——验收前置钉死可度量通过线;承认原型=已做的可行性spike;澄清定位=建标注数据集+校准评估器+评估基准(非必然fine-tune)。
- 产出 `REQ-training-eval.md`(系列1/4):目标/非目标、5个待拍板决策(路线A+C起步/T+1口径/ticker_hint命门等)、7条功能需求(含噪音真负例R4、防编数R7)、**验收前置**(A1数据质量门ticker准确率≥95%/A2留出集强度±1星≥80%方向≥90%推送precision≥75%recall≥70%/A3具名回归锚点美光不响+哨兵漏的响+无行情不出数/A4影子不劣化)、风险(相关≠因果RK1/ticker天花板RK3)、§5分工、开放问题。
- 待用户+V2审:D1路线/D4验收数值/留出集谁标/正式owner。审过V1再写②产品设计。
...
- **渠道决策 B 锁定 + 发现关注股冲突**：用户选 B(利空手机上限important,仅命中持仓/关注股才升级警笛)。核查发现 **GOOGL 在关注股内**(watchlist-state.md:41),若按 B 字面"利空命中关注股→升级"会把 cal-01 重新拉回 critical、撞验收锚点。故 B 加第2条闸:升级需「命中tracked **且** 已确认(非reportedly传闻)」——cal-01 是二手传闻→不升级→保持 important。SPEC §4 锁 B、§6 锚点标注该交互陷阱、SESSION 0c 更新。commit+push origin v1-stable。
- **推送门槛调整并入 SPEC**：用户看完现行评级标准(main prompt 全文)后拍板"强度3不上手机只上TG"=手机门槛 ≥3→≥4。因与 B 同属 `alert_level` 映射，并进同一 SPEC §4b：统一强度×方向→渠道表(★5利好critical/★4important/★3只上TG/≤2不推;利空按B降档)。实现触点标注(should_push语义别把3掉进no_push、dispatcher 复用现成 NOTABLE=TG-only-never-phone)。子选项待用户定:强度3的TG响铃(V1荐,防漏看RKLB)还是静音。连带更新 cal-01(强度3)alert_level important→notable(只上TG)、README门槛行×3、SESSION 0c。JSONL 9行绿。commit+push origin v1-stable。
- **子选项封口:强度3=静音TG**：用户选静音(不响铃)。强度3直接复用 dispatcher 现有 NOTABLE 档(silent TG/never phone),V2 无需新增档位;cal-01 alert_level=notable 已对上无需再改。SPEC §4b 锁定、映射表标注、SESSION 0c 更新。SPEC 三项决策(标尺方向中性/利空B/门槛≥4强度3静音TG)全部拍板闭合。commit+push origin v1-stable。
- **经验总纲 LESSONS.md + 隐忧核查交接 V2**：用户要求总结V1/V2经验并存档、隐忧告知V2。产出 `LESSONS.md`(第一原则=不假设对着代码核实 / A协作 B技术坑 C推送评级 D流程 E评估 / 隐忧节)。**核查隐忧修正判断**:dev_checklist 3failed/6errors 逐条查——6errors=ChromaDB Windows文件锁已知容忍;3failed全在test_scheduler=v1-stable分叉漂移(collector/scheduler.py差149/106,test_scraper_tick在main根本不存在)→**非main bug,是v1-stable攒旧代码**。如实纠正先前"测试红归V2修"的说法。出 `HANDOFF-lessons-concerns.md`:隐忧1=v1-stable漂移(请V2/用户按§2定port回main还是当草稿分支)、隐忧2=门禁容忍逻辑别吞真失败(真归main)、附LESSONS拉进main。commit+push origin v1-stable。
- **LESSONS §B 去重**：用户指出 LESSONS §B 与 TROUBLESHOOTING 双写。核实=约20%重叠(仅§B技术坑段),其余80%(协作/推送/流程/评估)TROUBLESHOOTING无。按自订"别双写"原则,把 §B 从复述改成纯索引表(坑名+去哪看修复,不抄修复内容)→每个事实一处真相。commit+push origin v1-stable。
- **关机存档**：更新 SESSION.md(本会话5项完成+0d新待办)。本会话共7次提交全推 origin/v1-stable(b782431→25ea62d)。给V2交接3份:SPEC-intensity-scale-bear-bias(推送标准三决策)、HANDOFF-lessons-concerns(隐忧核查)、LESSONS.md(经验总纲)。待V2/用户定:v1-stable scheduler漂移处置。
- **深度分析精简交接 → SPEC-deep-analysis-trim.md**：用户觉深度分析(有行情4步版)太长,逐轮定稿新格式(用防务军费 news_id=3787 做样例迭代)。规则:①定性精炼1-2句 ②传导只留直接链去间接/去加密货币 ③**组合映射修正**(原被泛化丢了"映射用户实仓";查出五大主承包商LMT/RTX/NOC/GD/BA不在用户关注,但HII/KTOS/AVAV/BWXT/PLTR在→③改指用户实际跟踪票)、单场景、方向词仅当有实时行情、禁价位与买卖操作、含1句反向风险 ④置信度1行、全文~250-300字。§4带定稿样例当回归锚点(3787)。强调不得回退anti-fabrication、须确认portfolio∪watchlist注入。归属main/V2。SESSION加0e。commit+push origin v1-stable。
