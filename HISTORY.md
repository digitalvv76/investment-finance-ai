...
## 2026-07-11 · v1-stable 归档重置 (V1窗口执行V2决策)
- V2决策(用户转达):v1-stable 累积113提交漂移(违反COLLAB §2),重置回干净main。
- 归档:archive/v1-stable-20260711 @ d209aee 已推origin(113提交全可恢复)。
- 重置:v1-stable → main(a009eac,=origin/main,线性)。
- 保留在建「训练评估项目」5文件(REQ-training-eval + external + HANDOFF-review + autolabel原型+测试)于干净分支,供V1继续②产品文档。
- 未提交HISTORY.md WIP已stash;main全程未被触碰。

---

## 2026-07-11 下午 · v1-stable窗口 · 卫生排查(撤销让位main) + 训练评估REQ过V2评审+核实BLOCKER

### 卫生4项(结论:让位trunk,不在v1-stable囤)
- 启动报4项(HISTORY缺6 hash / autolabel+eval_holdout未注册 / acceptance_test过时误报)。查出3项V2已在main主干做过(66046b6)→**v1-stable撤销我的改动避免冲突**(单主干原则)。acceptance_test"过时"=登记表把它错列deep_lane的related_script,实则不import→假依赖。
- deep_lane假依赖:我标出→V2修(bfbd26a@main),V2**多修我漏的第二张表**engine/__manifest__.json(pre_commit读)→记忆[[two-manifest-tables-sync]]。v1-stable未重做,下次对账继承。

### 训练评估REQ评审闭环(V1发起→V2评→V1核实)
- 定位方案:REQ-training-eval.md + external + HANDOFF-review + autolabel原型,均在v1-stable(已推origin/v1-stable)。HANDOFF本就是给V2的评审单。用户转达。
- V2评审(REVIEW-REQ-training-eval.md,fb36f7d@main)头号发现=**BLOCKER: event_driven决策完全不落库**。
- **我独立核实属实(v1-stable真实代码,同模型盲点铁律)**: `_apply_event_assessment`只写内存item.decision、return前不落库; ticker_hint/intensity/direction从不入表; impact_assessments schema缺全部字段; 无event_decisions表。→REQ默认的"从历史库挖训练数据"存储层做不到(R1/R3/R4/A2受影响)。
- 采纳V2建议方向: R0决策台账表(硬前置②设计前,V2 0.5天) + 不回填历史从落库日自增长 + 路线C先于A + 6实现坑。

### 待用户拍板(开机第一件)
1. 优先级:现在推进训练评估 vs 等生产三改观察1-2天完。
2. 接受"数据从落库日往前攒、不回填历史"吗。
- 定了→采纳R0+升REQ v0.2+写②产品设计。

### 信道
- V2→V1固定回执:`.claude/V2-TO-V1-HANDOFF.md` @ origin/main。

---

## 2026-07-12T09:53+08:00 · 会话开始

---

## 2026-07-12T20:46+08:00 · 关机同步

- 多LLM分工方案: 向用户说明 Agent/Workflow 两种模式, Fable/Opus/Sonnet/Haiku 四模型可用
- GLM Key排查: 查遍 .env / settings.json / memory — 系统中无 GLM/智谱 API Key, 仅历史对话讨论过(GL M5.1国际版$3/月)
- DeepSeek Key溯源: 来自 commit `8b29469` (2026-07-02), 随 multi-LLM 支持功能一起添加
- 事件升级模型健康检查: ECS news-monitor 容器 healthy(Up 30min), 5级管道正常, 看门狗报告健康(15条/时), 昨日推送活跃(最后一条 7/11 CoreWeave暴跌), 今日重启后零推送属周日低新闻流正常
