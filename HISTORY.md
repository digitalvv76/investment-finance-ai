...
## 2026-07-17 续 2 · T02 Agent 人格化 spec + 外部专家评估

### 外部专家评估
- 阅读了第三方对系统 brief 的评估 → 8/10 个人用，评分公允
- 专家三条建议中两条认可（数据可复现性 + 健康检查），一条过早（拆 worker/Redis）
- 时区统一追加到 V2 任务中

### 产出
- **Spec**: `docs/specs/SPEC-graham-review.md` — Graham 价值投资审查，5 问题清单，只能降级不能升级
  - 位置：Evaluate → Dispatch 之间
  - 触发：全部 push 决策
  - 降级规则：1-2 FAIL → 静音 TG，≥3 FAIL → 不推
  - 日调用 ~30-50 次，成本 < $0.01

### 决策
- Graham 审查模式：第二意见（B），全部推送覆盖（C），只能降级（A）
- 5 问题清单：预测/观点？→ 来源国影响美股？→ 改变盈利能力？→ 市场已消化？→ 明天回头看？

---

## 2026-07-17 续 · 广东CPI刷屏诊断 + 宏观地域三级体系 spec

### 诊断
- **TG 全天刷屏"广东居民消费价格"**: 五道防线全部失守——
  1. `居民消费价格` 命中 Tier A CPI 白名单 → 去重全部跳过
  2. 含 "CPI" 通过 US market signal → 中文降权不生效
  3. LLM Prompt 不区分美国 CPI vs 中国省级 CPI
  4. TG 没有主题去重（只有 Pushover 手机端有）
  5. 聚类窗口仅 30 分钟，跨时段无法合并

### 决策
- **宏观地域三级体系**（用户定）: T1 美国(×1.0) > T2 欧洲日本(×0.85) > T3 中国全国+韩国(×0.7) > 省级/市级排除

### 产出
- **Spec**: `docs/specs/SPEC-provincial-cpi-repeat-push.md` — P0 关键词收紧+P1 TG去重+P2 Prompt地域指令+P3 代码权重

---

## 2026-07-17 · AI 金融行业全景调研 + 开源方案评估

### 产出
- **AI 金融行业全景文档** (`docs/2026-07-17-01-ai-finance-landscape.md`): 覆盖 4 大板块——
  1. **10 个核心问题框架**：数据→模型→幻觉→时效性→评估→人机分工→多模态→个性化→合规→黑天鹅
  2. **行业优秀方案**：Bloomberg/BlackRock/Two Sigma/Man Group/D.E. Shaw/Kabra/Tengu 等
  3. **开源项目全景**：TradingAgents(92K⭐)/ai-hedge-fund(49K⭐)/Vibe-Trading(22K⭐)/FinGPT(20K⭐)/FinRobot/OpenAlice 等
  4. **5 个借鉴点可行性评估**：逐一分析引入代价和优先级

### 关键发现
- **通用 LLM 碾压金融专用模型**：BloombergGPT 已被 GPT-5.5/Claude Opus 4.7 全面超越，金融 NLP 不需要专用模型
- **架构模式已收敛**：四家对冲基金独立收敛到同一模式——隔离层+审计日志+人类审核+熔断
- **95% 基金在用 GenAI，但没有一家把 LLM 直接连到执行层**

### 决策
- **P0 立即做**：Agent 人格化（Graham 风格审查 prompt），堵同模型盲点
- **P1 立即做**：影子对比自动化脚本
- **P2 暂缓**：多 Agent 辩论（等人格化验证后再评估）
- **❌ 不做**：FinBERT 情感预筛选（任务不匹配 + 成本节约微小 + 增加复杂度）

### 踩坑
- 调研确认：同模型 agent 共享盲点是行业共识，差异化靠 prompt 视角而非模型不同
