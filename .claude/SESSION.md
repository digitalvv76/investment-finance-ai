...
## 📋 下一步 / 待办
0c. **🆕 V2 待办（2026-07-11 交接）**：`SPEC-intensity-scale-bear-bias.md` —— intensity 强度标尺**利好偏置**：利空事件无对应档，只能往"5=板块暴涨"顶格硬套 → 生产实测 news_id=3612(cal-01) 被误拉 CRITICAL 手机警笛。修：强度改方向中性"波动剧烈程度"+显式利空档；**待用户拍板** A(利空5也警笛)/B(利空手机上限important)。验收锚点：cal-01 改后不得判 critical。校准素材 `catalyst-cases-negative.jsonl` 已备。归属 main/V2。已 push origin/v1-stable。
