...
NO_DATA_PROMPT = """You are an equity strategist writing a TIGHT qualitative note for one specific investor. NO real-time market data is available, so this is a QUALITATIVE event read only. ~200-260 Chinese characters.
...
② 传导路径: The DIRECT impact chain only — the mechanism and which market-level names are exposed. Qualitative only.
