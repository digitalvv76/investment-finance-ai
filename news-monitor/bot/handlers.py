"""Telegram bot command and callback handlers (Chinese UI)."""
import asyncio
import json
import logging
...
def register_handlers(app: Application, db: Database, deep_lane=None, learner=None, curator=None, trainer=None) -> None:
    ...
    # /prefs
    # ------------------------------------------------------------------
    async def prefs_cmd(update, context):
        chat_id = update.effective_chat.id
        db.set_preference("telegram_chat_id", str(chat_id))
    ...
    # ------------------------------------------------------------------
    # /profile -- AI curator profile management
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # /analyze — analysis framework management
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # /alert — urgent interrupt keywords
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # /strategic — manage strategic alert rules
    # ------------------------------------------------------------------
    async def strategic_cmd(update, context):
        args = context.args or []
        chat_id = update.effective_chat.id
        db.set_preference("telegram_chat_id", str(chat_id))

        if not args:
            # Show current strategic rules
    ...
    # ------------------------------------------------------------------
    # /reason — scoring weight breakdown
    # ------------------------------------------------------------------
    async def reason_cmd(update, context):
        chat_id = update.effective_chat.id
        db.set_preference("telegram_chat_id", str(chat_id))

        # Get the most recently pushed news item
        ...
        # Decompose priority score
        # PriorityScorer factors: breaking(0.4) + tickers(0.06/ea) + macro(0.08/ea)
        # + people(0.05) + source_authority(0.03-0.10) + resonance(0.02/ea)
        ...
        # Show prediction accuracy stats if available
    ...
    async def handle_callback(update, context):
        ...
        if action in ('thumbs_up', 'thumbs_down'):
            action = 'content_good'  # Map old thumbs to content quality
        ...
        if action == 'content_good':
            ...
            # Record prediction accuracy stats (don't modify topic weights)
            try:
            ...
            # Legacy ignore — remove keyboard only
            await query.edit_message_reply_markup(reply_markup=None)
    ...
    app.add_handler(CommandHandler("strategic", strategic_cmd))
