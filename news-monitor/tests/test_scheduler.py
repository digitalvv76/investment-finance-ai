...
@pytest.mark.asyncio
async def test_tick_15min_fetches_rss(scheduler_setup):
    s = scheduler_setup["scheduler"]

    from storage.models import NewsItem
    items = [NewsItem(title="News", url="https://n.com/1", source="CNBC")]
    s.rss_fetcher.fetch_all.return_value = items
...
@pytest.mark.asyncio
async def test_tick_15min_empty(scheduler_setup):
    s = scheduler_setup["scheduler"]
    s.rss_fetcher.fetch_all.return_value = []
    await s._tick_15min()
    scheduler_setup["db"].insert_news.assert_not_called()
